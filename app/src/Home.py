...
if st.button("Act as Sara, a Movie Enjoyer", 
    ...
    st.session_state['role'] = 'movie_enjoyer'
    # we add the first name of the user (so it can be displayed on 
    # subsequent pages). 
    st.session_state['first_name'] = 'Sara'
    # finally, we ask streamlit to switch to another page, in this case, the 
    # landing page for this particular user type
    logger.info("Logging in as Movie Enjoyer Advisor Persona")
    st.switch_page('pages/00_Mov_Enjoy_Home.py')

if st.button('Act as Barbara, a Book Enjoyer', 
            type = 'primary', 
            use_container_width=True):
    st.session_state['authenticated'] = True
...
if st.button('Act as Rahul Shah, Journalist', 
            type = 'primary', 
            use_container_width=True):
    st.session_state['authenticated'] = True
